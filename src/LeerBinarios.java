...
public class LeerBinarios {
	public static void main(String[] args) throws IOException {
		...
		System.out.print("- Introduzca 'E' para escribir un fichero " + "\n" + "- Introduca 'L' para leer un fichero"
				...
				+ "- Introduca 'C' para leer un fichero binario: ");
		...
		if (opcion.equals("c") || opcion.equals("C")) {
			...
			if (fpath.equals(fichero.getPath())) {
				System.out.print("Introduce el numero de personas que quiere leer (maximo 3): ");
				numeroPersonas = input.nextInt();
				FileInputStream filein = new FileInputStream(fichero); // abro el stream de lectura

				i = 0;
				int actualPersonas = 0;

				// se pueda leer y no me pase del numero que tengo que leer
				while (actualPersonas < numeroPersonas && (i = filein.read()) != -1) // bucle para seguir
																						// almacenanndocuando es
				// < el numero introducido y que no esté vacio
				{
					String persona = "";
					persona += (char) i;

					int cont = 0;
					while (cont < 5 && (i = filein.read()) != -1) // bucle para continuar leyendo a traves de los saltos
																	// de linea
					{ // que hemos introducido en el ejercicio anterior
						if ('\n' == i) {
							cont++;
							persona += " ";
						} else {
